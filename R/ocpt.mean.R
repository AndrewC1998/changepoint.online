ocpt.mean.initialise <- function(
  ...
  if (minseglen < 1) {
    minseglen <- 1
    warning("Minimum segment length for a change in mean is 1, automatically changed to 1.")
  }
  ...
  ## Cost function choice (unchanged, still driven by penalty + test.stat)
  if (penalty == "MBIC") {
    if (test.stat == "Normal") {
  ...
  pen.num <- online.decision(
    ...
    n         = length(data),
    diffparam = 1,
    asymcheck = cost_func,   # matches existing use in Asymptotic branch
    method    = "AMOC"
  )
  
  method <- "PELT"
  
  ## Initialise online PELT with the numeric penalty
